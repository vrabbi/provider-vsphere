...
import (
	...
	"github.com/vrabbi/provider-vsphere/apis/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := creds[keyVsphereServer]; ok {
			ps.Configuration[keyVsphereServer] = v
		}
		if v, ok := creds[keyAPITimeout]; ok {
			ps.Configuration[keyAPITimeout] = v
		}
		if v, ok := creds[keyAllowUnverifiedSSL]; ok {
			ps.Configuration[keyAllowUnverifiedSSL] = v
		}
		return ps, nil
	}
}
